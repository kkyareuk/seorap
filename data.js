...
if(android225Followup){android225Followup.title="Android 229 시작 동기화·백업·태블릿·침대 실기기 확인";android225Followup.summary="최신 파일은 drawer-village-v1.0.212-code229-dev-debug.apk와 drawer-village-v1.0.212-code229-dev-release.aab입니다. code 224~228은 사용하지 마세요. Galaxy 휴대폰에서 로그인 전 첫 캐릭터를 만든 뒤 Google 로그인해 캐릭터가 유지·동기화되는지, 앱 재시작 때 동기화를 마친 뒤 게임이 열리는지, 설정의 백업 파일이 다운로드/DrawerVillage에 생성되는지 확인하세요. Galaxy SM-X700(Android 16)에서는 가로·세로 회전, 캐릭터 상세 패널, 가로 전체설정 양면 책, 메인 기준 집·마을 제목, 마을 장식 테두리 제거와 관찰 배치를 확인하세요. 침대의 두 얼굴·베개·이불·움직임·공유 카드, 로그 원형 프로필, 안테 찻잔 애니메이션, 다중 선택 요약 즉시 갱신도 함께 확인해야 해요. Play 테스트 트랙에 올릴 때는 Console의 최고 versionCode가 229보다 낮은지 먼저 확인하세요. 아직 Play 업로드나 Galaxy 실기기 확인은 하지 않았습니다.";}
const android219Followup=window.SEORAP_DATA.tasks.find(t=>t.id==="dv-android-214-device-play");
if(android219Followup){android219Followup.title="Android 221 프로덕션 핫픽스 실기기·Play 업로드 확인";android219Followup.summary="최신 파일 drawer-village-v1.0.204.2-code221-hotfix-release.aab. 220은 사용하지 마세요. Play 최고 코드가 221 미만인지 확인한 뒤 기존 219 앱 위에 업데이트하고 첫 캐릭터 생성·캐릭터 탭이 메인 화면을 먼저 여는지, 빠른설정과 전체설정이 분리됐는지, 빠른설정 뒤로가기·저장·글꼴, 전체설정의 메인 복귀, 캐릭터가 많은 계정의 탭 전환, 마을 산책과 발소리를 Galaxy 실기기에서 확인한 다음 프로덕션에 배포하세요.";}
