...
window.SEORAP_DATA.tasks.push({id:'dv-ios28',project:'서랍마을',section:'release',subsection:'testing',completed:false,title:'iOS 1.0.269(28) · 결제 응답 대기 보완',summary:'27에서도 Apple 결제창 응답대기 제보. dev8788817/iOS0a79906. UIKit SceneDelegate·단일창 manifest·Google/앱 URL 전달 유지, 활성windowScene 결제호출. 이전구매검증실패시 새결제차단·복원안내, 장기대기 상태확인(민감정보없음). 구매대기·영수증실패·복원회복 회귀통과. Mac iPad시뮬레이터 scene실행검사를 업로드전에 추가. Actions34338658465 성공. iPadOS26.2 창/루트/활성scene 검증 및 실행화면 확인. TestFlight28 나만테스트 그룹 배포완료. 실제기기28 구매대기 지속: hasScene/hasWindow/sceneActive=true, purchaseActive=true, waitingSeconds69, errorCode none. 실기기 재시동 테스트 요청, 결제해결은 미완료. Android302유지.'});

window.SEORAP_DATA.updates.unshift({version:'iOS 1.0.269 (28)',date:'2026-09-09',items:['iPad 결제창 연결 구조 및 이전 구매 검증 보완','장기 구매 대기 중 결제 상태 확인 추가','TestFlight 내부 배포 완료·iPadOS26.2 실행 검증 통과','사용자 iPad에서는 구매창 미표시 지속·진단 확인 후 추가 조사 중','Android302 유지·영어/일본어75.8%']});
window.SEORAP_DATA.tasks.push({id:'dv-command-menu303',project:'서랍마을',section:'development',subsection:'ui',completed:true,title:'활동 선택 · 취미 접근 및 메뉴 위계 개선',summary:'dev fb39474. 큰 분류 그림/설명·작은 활동 버튼·경로와 캐릭터명 구분. 취미 목록 진입 복원·등록 취미 표시·자동 취미 실행·빈 설정 안내와 직접 선택. 대화상자 열린 동안 배경 재렌더 지연·닫은 뒤 반영. 360x840/1180x820 실행 및 잘림 검사 통과·실제 자동 취미 명령 생성 확인. 신규 문구 KO/EN/JA 완료. 개발 코드 반영, 설치파일/TestFlight28에는 아직 미포함. Android versionCode302 유지.'});
