...
window.SEORAP_DATA.updates=[
  {version:"1.0.207-dev",code:224,date:"2026-09-05",items:["Android 태블릿을 세로로 고정하고 모든 화면을 480px 휴대폰 레이아웃으로 통일","캐릭터 화면도 휴대폰과 같은 주민등록증·빠른설정·전체설정 흐름을 사용해 태블릿 오른쪽 내용이 비던 오류 제거","관찰 화면 캐릭터 이름과 마을 화면 집 이름의 흰색 글씨 크기·위치를 홈 화면 기준으로 정렬","223까지의 찻잔·음식 애니메이션, 다중 선택 요약 즉시 갱신, 첫 캐릭터 로그인 보존 수정 포함","224 전용·캐릭터 UI·태블릿·기존 제보·선택 연속성·로그인 계정 격리 검사와 웹/Android 준비, APK/AAB 서명·214개 자산 해시 검증 통과","한국어·영어·일본어 출시노트 완료. dev 준비판이며 운영 main·사이트·Play 배포는 변경하지 않음. Galaxy SM-X700 실기기 확인은 남음. 전체 UI 정적 번역 영어 75.2%, 일본어 75.1%"]},
