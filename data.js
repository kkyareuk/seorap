...
window.SEORAP_DATA.tasks.push({id:"dv-group-multiplayer-mvp",project:"서랍마을",section:"release",subsection:"testing",completed:false,title:"그룹 멀티 1차 설계·구현",summary:"일반적인 용어인 그룹·멤버·관리자·운영자를 사용해 비공개 공동생활 멀티를 만들어요. 그룹 안에 여러 마을과 집이 있고 멤버는 자신이 보유한 캐릭터를 그룹 규칙이 허용한 수만큼 추가하며 같은 그룹의 마을·집을 방문할 수 있어요. 그룹 관리자는 멤버별 캐릭터 상한과 방문·관계·운영 규칙을 정하고, 관리자·운영자에게는 별도 또는 무제한 상한을 둘 수 있어요. 관리자가 캐릭터를 제거할 때는 사용자의 원본을 삭제하지 않고 그룹 주민 등록과 그룹 내 배치만 해제하며 기록 보존·재가입 정책을 명시해요. 첫 단계는 그룹 생성·코드 가입·역할/권한·캐릭터 추가/제거·마을/집 방문·소유권 보호를 확정하고, 이후 공동 생활 로그와 관계 자동 발전을 연결해요."});
const android225Followup=window.SEORAP_DATA.tasks.find(t=>t.id==="dv-android-223-device-play");
if(android225Followup){android225Followup.title="Android 233 태블릿·백업·계정 보호 실기기 확인";android225Followup.summary="최신 개발 파일은 drawer-village-v1.0.215-code233-dev-debug.apk와 drawer-village-v1.0.215-code233-dev-release.aab입니다. code 224~232 개발 파일 대신 233을 사용하세요. SM-X700(Android 16)에서는 캐릭터 목록 오른쪽에 선택한 캐릭터 정보가 즉시 보이는지, 가로 전체설정의 큰 양면 책과 세로의 큰 단면 책, 관찰·마을의 전체 화면 채움과 오른쪽 여백 제거, 프로필 옆 이름·직업 정렬, 마을 캐릭터·장식의 흰 배경과 테두리 제거를 확인하세요. 설정의 백업 내보내기에서 Android 파일 저장창이 열리고 선택한 위치에 JSON이 저장되는지, 첫 로그인 전 캐릭터가 로그인 뒤에도 유지·동기화되는지도 확인해야 해요. Play 테스트 트랙에 올릴 때는 Console의 최고 versionCode가 233보다 낮은지 먼저 확인하세요. 아직 Play 업로드나 Galaxy/태블릿 실기기 확인은 하지 않았습니다.";}
const android219Followup=window.SEORAP_DATA.tasks.find(t=>t.id==="dv-android-214-device-play");
if(android219Followup){android219Followup.title="Android 231 Google 로그인 핫픽스 실기기·Play 업로드 확인";android219Followup.summary="최신 운영 파일은 drawer-village-v1.0.204.3-code231-hotfix-release.aab입니다. code 221 이하 파일 대신 231을 사용하세요. Play Console의 현재 최고 versionCode가 231보다 낮은지 확인한 뒤 기존 앱을 삭제하지 말고 업데이트하세요. Android 17 Galaxy에서 설정의 Google 로그인 버튼을 누르면 진행 안내와 계정 선택창이 즉시 열리는지, 로그인 전 만든 캐릭터가 유지·동기화되는지, 동기화·불러오기와 상점 캐릭터 슬롯 구매가 로그인 계정으로 진행되는지 확인하세요. 앱 삭제나 데이터 초기화는 기기에만 있는 캐릭터를 잃을 수 있으므로 하지 마세요. Play 업로드와 Galaxy 실기기 최종 확인은 아직 하지 않았습니다.";}
