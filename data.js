...
if(android219Followup){
  android219Followup.title="Android 240 홈 소리·캐릭터 배치 핫픽스 Play 업로드·실기기 확인";
  android219Followup.summary="현재 Google Play 프로덕션은 1.0.215(code 233)이고, 업로드할 최신 운영 파일은 drawer-village-v1.0.215.3-code240-hotfix-release.aab입니다. 기존 앱을 삭제하거나 저장공간 데이터를 지우지 말고 Play 업데이트로 설치하세요. SM-A346N 등 휴대폰 홈 관찰 화면에서 마을 주민 이동음이 들리지 않는지, 캐릭터 설정의 배치 저장 버튼과 실제 홈 적용이 정상인지 확인하세요. 관계 시선 설정 마지막 항목 스크롤, 로그인 전 만든 캐릭터의 Google 로그인 뒤 유지·동기화, 백업 내보내기와 상점도 함께 확인하고 문제가 남으면 code 240 설치 뒤 생성된 자동 진단 정보를 받아야 해요. Play 업로드와 Galaxy 실기기 최종 확인은 아직 하지 않았습니다.";
}
